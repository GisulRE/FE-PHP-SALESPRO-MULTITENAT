...
CREATE VIEW `kardex` AS
    SELECT 
        ...
        `record`.`product_qty_before` AS `product_qty_before`,
        `record`.`product_qty_after` AS `product_qty_after`,
        ...
        CASE
            ...
            WHEN `record`.`transaction_type` = 1 THEN `product_sales`.`cost`
        ...
        END AS `cost`,
        CASE
			WHEN `record`.`transaction_type` = 0 THEN  format(`record`.`warehouse_qty_after` * `products`.`cost`, 2)
            WHEN `record`.`transaction_type` = 1 THEN  format(`product_sales`.`qty` * `product_sales`.`cost`, 2)
            WHEN `record`.`transaction_type` = 2 THEN  format(`product_purchases`.`qty` * `product_purchases`.`net_unit_cost`, 2)
            WHEN `record`.`transaction_type` = 3 THEN  format(`products`.`cost` * `product_returns`.`qty`, 2)
            WHEN `record`.`transaction_type` = 4 THEN  format(`product_transfer`.`qty` * `product_transfer`.`net_unit_cost`, 2)
            WHEN `record`.`transaction_type` = 5 THEN  format(`products`.`cost` * `product_adjustments`.`qty`, 2)
            WHEN `record`.`transaction_type` = 6 THEN  format(`purchase_product_return`.`qty` * `purchase_product_return`.`net_unit_cost`, 2)
            ELSE NULL
        END AS `total_cost`,
        CASE
            WHEN `record`.`transaction_type` = 4 AND `record`.`warehouse_qty_before` < `record`.`warehouse_qty_after` THEN `transfers`.`from_warehouse_id`
            WHEN `record`.`transaction_type` = 4 AND `record`.`warehouse_qty_before` > `record`.`warehouse_qty_after` THEN `transfers`.`to_warehouse_id`
            ELSE NULL
        END AS `from_to`,
    ...
    FROM
		...
		LEFT JOIN `return_purchases` 
			ON `record`.`transaction_type` = 6
			AND `record`.`transaction_id` = `return_purchases`.`id`
        JOIN `users` 
			ON (`record`.`transaction_type` = 0 AND 1 = `users`.`id`)
			OR (`record`.`transaction_type` = 1 AND `sales`.`user_id` = `users`.`id`)
			OR (`record`.`transaction_type` = 2 AND `purchases`.`user_id` = `users`.`id`)
			OR (`record`.`transaction_type` = 3 AND `returns`.`user_id` = `users`.`id`)
			OR (`record`.`transaction_type` = 4 AND `transfers`.`user_id` = `users`.`id`)
			OR (`record`.`transaction_type` = 5 AND 1 = `users`.`id`)
            OR (`record`.`transaction_type` = 6 AND `return_purchases`.`user_id` = `users`.`id`);