...
class StockCountController extends Controller
{
        ...
        if( count($lims_product_list) ){
            ...
            foreach ($lims_product_list as $product) {
                if($product->variant_id){
                    $lims_product_variant_data = ProductVariant::select('item_code')->FindExactProduct($product->id, $product->variant_id)->first();
                    $csvData[]=$product->name.','.$lims_product_variant_data->item_code.','.$product->qty.','.'';
                }else{
            ...
            return redirect()->back()->with('message', 'Conteo de stock creado con éxito! Por favor descargue el archivo inicial para completar despues.');
        }
        else
            return redirect()->back()->with('not_permitted', 'Productos no encontrados!');
    }
    ...
    public function finalize(Request $request)
    {
        ...
        return redirect()->back()->with('message', 'Conteo de stock finalizado con éxito!');
    }
            ...
            if( $current_line && $i > 0 && ($current_line[2] != $current_line[3]) ){
                ...
                if(!$product_data) {
                    $product_data = Product::join('product_variants', 'products.id', 'product_variants.product_id')
                        ...
                        ->where('product_variants.item_code', $current_line[1])
                        ->first();
                }
                if($current_line[3]){
