        ...
        <div id="receipt-data">
            ...
            <table>
                ...
                <tfoot style="border-collapse: unset;">
                    <tr>                        
                        <th colspan="5" style="text-align:right">{{ trans('file.Total') }} {{ $general_setting->currency }}:
                            {{ number_format((float) $lims_sale_data->total_price, 2, '.', ',') }}</th>
                    </tr>
                    @if ($lims_sale_data->order_tax)
                        <tr>                            
                            <th colspan="5" style="text-align:right">{{ trans('file.Order Tax') }} {{ $general_setting->currency }}:
                                {{ number_format((float) $lims_sale_data->order_tax, 2, '.', ',') }}</th>
                        </tr>
                    @endif
                    @if ($lims_sale_data->order_discount)
                        <tr>                            
                            <th colspan="5" style="text-align:right">{{ trans('file.Order Discount') }} {{ $general_setting->currency }}:
                                {{ number_format((float) $lims_sale_data->order_discount, 2, '.', ',') }}</th>
                        </tr>
                    @endif
                    @if ($lims_sale_data->coupon_discount)
                        <tr>                            
                            <th colspan="5" style="text-align:right">{{ trans('file.Coupon Discount') }} {{ $general_setting->currency }}:
                                {{ number_format((float) $lims_sale_data->coupon_discount, 2, '.', ',') }}</th>
                        </tr>
                    @endif
                    @if ($lims_sale_data->shipping_cost)
                        <tr>                            
                            <th colspan="5" style="text-align:right">{{ trans('file.Shipping Cost') }} {{ $general_setting->currency }}:
                                {{ number_format((float) $lims_sale_data->shipping_cost, 2, '.', ',') }}</th>
                        </tr>
                    @endif
                    <th colspan="5" style="text-align:right">{{ trans('file.grand total') }} {{ $general_setting->currency }}: {{ number_format((float) $lims_sale_data->grand_total, 2, '.', ',') }}</th>
                    <tr>
