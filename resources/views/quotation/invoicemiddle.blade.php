...
<head>
    ...
    <style type="text/css">
        @font-face {
            ...
            src: url('public/fonts/BPdots-webfont.eot');
            src: url('public/fonts/BPdots-webfont.eot?#iefix') format('embedded-opentype'),
                url('public/fonts/BPdots-webfont.woff2') format('woff2'),
                url('public/fonts/BPdots-webfont.woff') format('woff'),
                url('public/fonts/BPdots-webfont.ttf') format('truetype'),
                url('public/fonts/BPdots-webfont.svg#bpdotsregular') format('svg');
            font-weight: normal;
            font-style: normal;
...
<body>
    ...
    <div class="invoice-ticket-container" style="max-width:100%;margin:0 auto">
        ...
        <div id="receipt-data">
            <div class="centered">
                <h1 class="mb-1" style="font-size:20px">PROFORMA</h1> {{ $general_setting->site_title }} <br> Telf:
                {{ $lims_quotation_data->warehouse->phone }}
            </div>
            <br>
            <table>
                <tr>
                    <td style="width: 6%">
                        <div class="left">
                            @if ($general_setting->site_logo)
                                <img src="{{ url('public/logo', $general_setting->site_logo) }}" height="42"
                                    width="42" style="margin:10px 0;">
                            @endif
                        </div>
                    </td>
                    <td style="width: 45%">
                        <div class="">
                            {{ trans('file.reference_quotation') }}: <span
                                style="font-size: 15px;font-weight: bold;">{{ $lims_quotation_data->reference_no }}</span><br>
                            {{ trans('file.Date') }}:
                            {{ date($general_setting->date_format, strtotime($lims_quotation_data->created_at->toDateString())) }}<br>
                            {{ trans('file.Biller') }}: {{ $lims_biller_data->name }}<br>
                            @if ($lims_quotation_data->quotation_status == 1)
                                {{ trans('file.Status') }}: {{ trans('file.Pending') }}<br>
                            @elseif ($lims_quotation_data->quotation_status == 2)
                                {{ trans('file.Status') }}: {{ trans('file.Sent') }}<br>
                            @endif
                        </div>
                    ...
                    <td style="width: 45%">
                        <div class="">
                            {{ trans('file.customer') }}: {{ $lims_quotation_data->customer->name }}
                            <br>{{ trans('file.Address') }}: {{ $lims_quotation_data->customer->address }}
                            <br>{{ trans('file.Phone Number') }}: {{ $lims_quotation_data->customer->phone_number }}
                            @if ($lims_quotation_data->valid_date != null)
                                <br>{{ trans('file.date_valid') }}: {{ date($general_setting->date_format, strtotime($lims_quotation_data->valid_date)) }}
                            @endif
                        </div>
                    </td>
                </tr>
            ...
            <table>
                <thead>
                    ...
                    <th style="text-align:center">{{ trans('file.Discount') }}</th>
                ...
                <tbody>
                    @foreach ($lims_product_quotation_data as $key => $product_quotation_data)
                        ...
                        <tr>
                            <td style="text-align:center">{{ $product_quotation_data->qty }}</td>
                            ...
                            <td style="text-align:right;">
                                {{ number_format((float) $product_quotation_data->discount, 2, '.', ',') }}
                            </td>
                            <td style="text-align:right;vertical-align:bottom">
                                {{ number_format((float) $product_quotation_data->total, 2, '.', ',') }}</td>
                        </tr>
                    @endforeach
                </tbody>
                <tfoot style="border-collapse: unset;">
                    <tr>
                        <th colspan="4"></th>
                        <th colspan="2" style="text-align:right">&nbsp;&nbsp;&nbsp;{{ trans('file.Total') }}:
                            {{ number_format((float) $lims_quotation_data->total_price, 2, '.', ',') }}</th>
                    </tr>
                    @if ($lims_quotation_data->order_tax)
                        <tr>
                            <th colspan="4"></th>
                            <th style="text-align:right">{{ trans('file.Order Tax') }}:
                                {{ number_format((float) $lims_quotation_data->order_tax, 2, '.', ',') }}</th>
                        </tr>
                    @endif
                    @if ($lims_quotation_data->order_discount)
                        <tr>                            
                            <th colspan="6" style="text-align:right">{{ trans('file.Order Discount') }}:
                            {{ number_format((float) $lims_quotation_data->order_discount, 2, '.', ',') }}</th>
                        </tr>
                    @endif
                    @if ($lims_quotation_data->shipping_cost)
                        <tr>
                            <th colspan="4"></th>
                            <th style="text-align:right">{{ trans('file.Shipping Cost') }}:
                                {{ number_format((float) $lims_quotation_data->shipping_cost, 2, '.', ',') }}</th>
                        </tr>
                    @endif
                    <tr>
                        ...
                        @else
                            <th class="centered" colspan="5">{{ trans('file.In Words') }}:
                                <span>{{ str_replace('-', ' ', $numberInWords) }}</span>
                                <span>{{$cadenaCentavos}} {{ $general_setting->currency }}</span>
                            </th>
                        @endif
                        <th style="text-align:right">
                            {{ trans('file.grand total') }}:{{ number_format((float) $lims_quotation_data->grand_total, 2, '.', ',') }}
                        </th>
                    </tr>
                </tfoot>
            </table>
