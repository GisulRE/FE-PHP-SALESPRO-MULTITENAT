...
SQL
            ...
            DB::unprepared('DROP TRIGGER IF EXISTS after_product_sale_trigger ON product_sales;');
            DB::unprepared(<<<'SQL'
CREATE OR REPLACE FUNCTION after_product_sale_trigger_fn()
RETURNS trigger AS $$
DECLARE
    trans_id integer;
...
CREATE TRIGGER after_product_sale_trigger
AFTER INSERT ON product_sales
FOR EACH ROW
EXECUTE FUNCTION after_product_sale_trigger_fn();
SQL
            );
        }
    }
    ...
    public function down()
    {
        ...
        if ($driver === 'mysql') {
            ...
            DB::unprepared('DROP TRIGGER IF EXISTS after_product_sale_trigger ON product_sales;');
            DB::unprepared('DROP FUNCTION IF EXISTS after_product_sale_trigger_fn();');
        }
    }
}
